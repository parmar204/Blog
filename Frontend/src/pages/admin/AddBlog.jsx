...
const AddBlog = () => {
  ...
  const handlerSubmit = async (e) => {
    ...
    try {
      setIsAdding(true)
      const blog = {
        title, subtitle, category, description: quillRef.current.root.innerHTML, isPublished
      }
  ...
  return (
    <form onSubmit={handlerSubmit} className='flex-1 bg-blue-50/50 text-gray-600 h-full overflow-scroll'>
      <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded'>
        ...
        <div className='flex gap-2 mt-4'>
          <p>Publish Now</p>
          <input type="checkbox" checked={isPublished} value={isPublished} className='scale-125 cursor-pointer' onChange={e => setIsPublished(e.target.checked)}/>
        </div>
